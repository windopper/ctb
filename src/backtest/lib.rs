...
impl BacktesterState {
    ...
    pub fn check_and_close_position(&mut self, current_price: f64) {
        if let PositionState::InPosition { entry_price, take_profit_price, trailing_stop_price } = self.position {
            let mut pnl_pct = 0.0;
    ...
    pub fn handle_signal(&mut self, signal: &Signal, current_price: f64) {
        if let PositionState::None = self.position {
            if let Signal::Buy { reason, initial_trailing_stop, take_profit } = signal {
                ...
                println!("\x1b[34m[진입] 가격: {:.0}, 목표가: {:.0}, 트레일링스탑: {:.0}, 이유: {}\x1b[0m", 
                        current_price, take_profit, trailing_stop_price, reason);
            }
        }
        ...
        if let PositionState::InPosition { entry_price, .. } = self.position {
            if let Signal::Sell(reason) = signal {
                ...
                println!("\x1b[35m[전략 매도] 진입가: {:.0}, 현재가: {:.0}, 실현 손익: {:.4}%, 이유: {}\x1b[0m", 
            ...
            if let Signal::UpdateTrailingStop(new_trailing_stop) = signal {
                if let PositionState::InPosition { entry_price, take_profit_price, trailing_stop_price: _ } = self.position {
                    self.position = PositionState::InPosition {
                        entry_price,
                        take_profit_price,
